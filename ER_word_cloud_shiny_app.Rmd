...
ui <- fluidPage(
  ...
  fluidPage(fluidRow(column(width=10, offset = 2),
                     h2("Top Columns"),
...
server <- function(input, output) {
  ...
  output$topicCorpusWeight <- renderText({ 
    ...
    topicweight <- topicweight$V2 * 100
  ...
  output$topic_plot <- renderPlot({  
    ...
    output$time_graph <- renderPlotly({
      exploretopics_byyear <- function(topic) {
        ...
        tby_plot <- ggplot(filterbytopicnum, aes(year, mean_value * 100)) + geom_bar(stat="identity") + scale_x_continuous(name="Year", breaks=seq(1935, 1962, 2)) + scale_y_continuous(limits=c(0,5))
tby_plot    }
...
leaflet(tm_1) %>%
  ...
  addCircleMarkers(radius = ~(value) *50,
                   color = ~pal(topicnum),
                   stroke = FALSE,
                   label = ~paste("Topic Number = ", topicnum, " | ", "Percent = ", str_sub(value, 1, 4), "%", " | ", Columndate, "|", " Place Name: ", PlaceName, sep = ""),
                   fillOpacity = 0.3) %>%
  addLegend("topright", pal = pal, values = ~topicnum)
```
